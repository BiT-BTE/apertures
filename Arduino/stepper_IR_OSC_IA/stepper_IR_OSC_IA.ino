...
void setup() {
  ...
  stepper1.setAcceleration(20.0);  // steps/s^2
...
void loop() {
  ...
  // Mantener los steppers vivos lo antes posible
  stepper1.run();
  stepper2.run();
  ...
  if (packetSize > 0) {
    ...
    while (Udp.available()) {
      ...
      // Importante: seguir atendiendo los motores mientras entra el paquete
      stepper1.run();
      stepper2.run();
    }

    if (!msg.hasError()) {
      handleOSC(msg);  // (asumiendo que aquí solo haces moveTo/move y NO bloqueas)
    } else {
      Serial.println("Error en mensaje OSC.");
    }
  }

  // Volver a llamar run() (barato y ayuda)
  stepper1.run();
  stepper2.run();
  ...
  // Enviar OSC a tasa fija (evita saturación y mantiene loop rápido)
  static unsigned long lastSend = 0;
  const unsigned long sendIntervalMs = 40;  // 25 Hz (ajusta: 20ms=50Hz, 100ms=10Hz)
  ...
  stepper1.run();
  stepper2.run();
  // Si ambos motores ya llegaron, deshabilitar driver
...
void handleOSC(OSCMessage& msg) {
  ...
  // Asegura que los motores estén habilitados cuando llegue un comando
  digitalWrite(enablePin, LOW);

  // /M1 <float>
  if (msg.fullMatch("/M1") && msg.size() >= 1) {
    ...
    stepper1.moveTo(targetPos);  // <-- NO bloquea

    Serial.println("M1 objetivo seteado.");
  }

  // /M2 <float>
  else if (msg.fullMatch("/M2") && msg.size() >= 1) {
    float revs = msg.getFloat(0);
    ...
    stepper2.moveTo(targetPos);  // <-- NO bloquea

    Serial.println("M2 objetivo seteado.");
  }

  // /MB <float> <float>
  else if (msg.fullMatch("/MB") && msg.size() >= 2) {
    ...
    stepper1.moveTo(targetPos1);  // <-- NO bloquea
    stepper2.moveTo(targetPos2);  // <-- NO bloquea

    Serial.println("MB objetivos seteados.");
  }

  else {
    Serial.println("Dirección OSC desconocida (usa /M1 /M2 /MB).");
  }
}
